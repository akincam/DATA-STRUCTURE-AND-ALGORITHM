...
// Insert solution to programming exercise 1, section 5, chapter 9 here
    ...
    /**
     * looks logn nanotime whether item is in Tree
     * if item is given middle element continues right array
     * else ccontinues left array
     * @param item is element method searchs this element in BTree
     * @param data searchable array
     * @param i small index=0
     * @param size array size
     * @return index if item is in the array else returns index which range it is in
     */
    private int binarySearch(E item, E[] data, int i, int size) {
        ...
        while(count!=size/2+1) {
            int median=(large+small)/2;
            int comp=item.compareTo(data[median]);
            if (comp > 0) {
                index=median+1;
                small=median+1;
                count++;
    ...
    /**
     * Searchs item in tree
     * @param item is element method searchs this element in BTree
     * @return item if finds item else return null
     */
    public E binarySearch(E item ){
        E returnV=binarySearch(item,root);
        System.out.println(index);
        return returnV;
    }

    /**
     * if given node is null(empty Tree) return null
     * else finds index
     * if item is equal is given index return item and assings its location in index instance variable
     * else looks whether is child null
     * if child is null return null index is -1
     * else calls method with child node
     * @param item is element method searchs this element in BTree
     * @param localRoot base node is root and continue root.child ...
     * @return item if finds item else return null
     */
    private E binarySearch(E item,Node<E> localRoot){
        if(localRoot==null){
            index=-1;
            return null;
        }
        int num=binarySearch(item,localRoot.data,0,localRoot.size);
        if(num!=localRoot.size&&item.compareTo(localRoot.data[num])==0){
            index=num;
            return localRoot.data[num];
        }
        if(localRoot.child[num]==null) {
            index = -1;
            return null;
        }
        else {
            return binarySearch(item, localRoot.child[num]);
        }
    }
}